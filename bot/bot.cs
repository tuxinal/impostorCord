...
namespace Impostor.Plugins.ImpostorCord.Discord
{
    ...
    public class Bot
    {
        ...
        static async Task MainAsync(string token)
        {
            ...
            {
                if(e.Before.Channel != null && e.After.Channel == null){ // if they were in a channel BEFORE and they are not in a channel AFTER
                    foreach(KeyValuePair<string,Game> game in games){
                        if(game.Value.voiceChannel == e.Before.Channel){ //checking through all games to find the one connected to the vc
                            ...
                            int count = 0;
                            foreach(Player player in game.Value.players){ //getting through each player to find the one connected to the member
